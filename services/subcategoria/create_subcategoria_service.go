...
func CreateSubCategoriaService(idUsuario int, request schemas.CreateSubCategoriaRequest) (*schemas.SubCategoriaResponse, error) {

	// Normaliza o nome removendo acentos e convertendo para maiúsculo
	request.Nome = utils.NormalizarString(request.Nome)

	subcategoriaEntity := SchemaCreateToEntity(request)
